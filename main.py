...
from scripts import combinations, readV, getFeatures, dict, singleSTA, utils, extData, dir, getArea, makeCSV, capacitance
...
to_run = dir.get_files(verilogs_inputs)

for file in to_run:

    f_str = file.split(".v")[0]
    design = f_str
    
    try:
        fanouts_topology = capacitance.Netlist_fouts(f"{lib_dir}/{lib}", f"{verilogs_inputs}/{design}.v")
        map_fanouts = fanouts_topology.direct_fanout_map()
        # pprint.pprint(map_fanouts)  # Comentado para não poluir o terminal, se quiser pode descomentar
    except Exception as error:
        print(f"ERROR TO MAP FANOUTS:\n{error}")
        sys.exit(1)
    ...
    processor = process.CellProcessor(
        ...
        csv_path=csv_path,
        json_capacitance=f"{lib_dir}/capacitance.json",
        map_fanouts=map_fanouts,
    )
